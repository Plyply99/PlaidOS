...
dnf5 -y --setopt=obsoletes=0 install gstreamer1-vaapi
...
### Install packages
#dnf5 -y install gdm akmods steam-devices distrobox mpv input-remapper mangohud vkBasalt adw-gtk3-theme bat bat-extras btop cava chafa emacs eza fastfetch gamemode ghostty htop xwininfo libva-utils rpmdevtools kitty nethogs iotop amdgpu_top
...
dnf5 -y install nautilus gnome-software gnome-software-rpm-ostree steam-devices
...
dnf5 -y install cups cups-filters system-config-printer gdm gutenprint gutenprint-cups bluez-cups NetworkManager-wifi avahi-dnsconfd firewalld firewall-offline-cmd distrobox smartmontools speech-dispatcher amdgpu_top
dnf5 -y install adw-gtk3-theme bat bat-extras btop chafa cliphist dgop dms dsearch emacs eza fastfetch gamemode ghostty gnome-disk-utility grim input-remapper mangohud mpv nautilus niri npm nwg-look python3-dbus-next qt6-qtmultimedia slurp vkBasalt
dnf5 -y install hyprland hyprland-guiutils hyprpicker #--allowerasing --skip-broken

# Remove niri-git and hyprland optional dependencies
dnf5 -y remove fuzzel mako swaybg swayidle swaylock SwayNotificationCenter waybar wofi
dnf5 clean all
